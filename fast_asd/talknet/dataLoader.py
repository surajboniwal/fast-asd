...
def load_visual(data, dataPath, numFrames, visualAug):
    ...
    sortedFaceFiles = sorted(
        faceFiles, key=lambda data: float(data.split("/")[-1][:-4]), reverse=False
    )
    faces = []
    H = 112
    if visualAug == True:
...
class train_loader(object):
    ...
    def __getitem__(self, index):
        ...
        audioSet = generate_audio_set(
            self.audioPath, batchList
        )  # load the audios in this batch to do augmentation
        for line in batchList:
            data = line.split("\t")
            audioFeatures.append(
                load_audio(
                    data, self.audioPath, numFrames, audioAug=True, audioSet=audioSet
                )
            )
            visualFeatures.append(
                load_visual(data, self.visualPath, numFrames, visualAug=True)
            )
...
class val_loader(object):
    ...
    def __getitem__(self, index):
        ...
        audioFeatures = [
            load_audio(
                data, self.audioPath, numFrames, audioAug=False, audioSet=audioSet
            )
        ]
        visualFeatures = [
            load_visual(data, self.visualPath, numFrames, visualAug=False)
        ]
